...
class User extends Main
{
     ...
     */
    public static function hasPermission(App $object, Entity $user, $name): bool
    {
        $data = false;
        $user_permissions = [];
        $uuid = $user->getUuid();
    ...
    private static function getTokens(App $object, Entity $node): array
    {
        ...
        $toArray = \Host\Api\Workandtravel\World\Service\Entity::getToArray(
            $object,
            $entity,
            $entity . '.' . $function . '.toArray'
        );
        $record = [];
        $record = \Host\Api\Workandtravel\World\Service\Entity::toArray(
            $object,
            $node,
            $toArray,
            $entity,
